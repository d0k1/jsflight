...
Xpath.getElementTreeXPath = function(element) {
	...
	for (; element && element.nodeType == Node.ELEMENT_NODE; element = element.parentNode) {
		...
		for (var sibling = element.nextSibling; sibling
				&& !hasFollowingSiblings; sibling = sibling.nextSibling) {
			if (sibling.nodeName == element.nodeName)
	...
	while (rule.length && rule != lastRule) {
		...
		if (m) {
			if (!m[1]) {
				...
				parts
						.push("[contains(concat(' ',normalize-space(@class),' '), ' "
								+ m[2] + " ')]");
		...
		// Skip over pseudo-classes and pseudo-elements, which are of no use to
		// us
 ...
 *            resultType
 * 
 * @returns {*} The result of the XPath expression, depending on resultType :<br>
 *          <ul>
 ...
 *          <li>if it is XPathResult.UNORDERED_NODE_ITERATOR_TYPE or
 *          XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, then it returns an array of
 *          nodes</li>
 *          <li>if it is XPathResult.ORDERED_NODE_SNAPSHOT_TYPE or
 *          XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, then it returns an array
 *          of nodes</li>
 *          <li>if it is XPathResult.ANY_UNORDERED_NODE_TYPE or
 *          XPathResult.FIRST_ORDERED_NODE_TYPE, then it returns a single node</li>
 *          </ul>
 */
