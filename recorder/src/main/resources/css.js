...
function initPropertyData()
{
    ...
    var addMissingValues = function(values, propName)
    {
        ...
        // "currentColor", system colors, Mozilla-specific colors, see bug
		// 927367
        if (values.indexOf("aqua") !== -1)
            values = values.concat(extraColors);

        // Gradients, see bug 973345
        // xxxsz: Can be removed when Firefox 31 is the minimum supported
		// version
    ...
    for (let prop of props)
    {
        ...
        // See if the property is special-cased due to missing or incorrect
		// values.
...
{
    // Currently getCSSPropertyKeywordsExcludingCategories and
	// getCSSShorthandCategory
    // only support background, border and font, assert that nothing else gets
	// passed
 ...
 * 
 * @param {Object}
 *            style CSSStyleRule to get the properties of
 * @param {Object}
 *            element Element to which the style applies. Needed for parsing
 *            shorthand properties correctly.
 * 
 * @returns {Array} Properties represented by {name, value, priority,
 *          longhandProps}
 */
...
Css.removeClass = function(node, name)
{
    ...
    else // XXXsroussey don't cache these, they are often setting values.
			// Should be using setUserData/getUserData???
        re = new RegExp('(^|\\s)' + name + '(\\s|$)', "g");
...
Css.setClassTimed = function(elt, name, context, timeout)
{
    ...
    if (elt.__setClassTimeout)  // then we are already waiting to remove the
								// class mark
    ...
    elt.__setClassTimeout = context.setTimeout(function()
    {
        ...
        if (elt.__invisibleAtSetPoint)  // then user can't see it, try again
										// later
...
Css.isValidStylesheet = function(styleSheet)
{
    try
    {
        // See if the getter triggers an exception. "void" is there to silence
		// jshint.
...
Css.createStyleSheet = function(doc, url)
{
    ...
    if (cssText)
    {
        ...
        // Note that stylesheets can come from various extensions and the source
		// can
...
Css.getStyleSheetByHref = function(url, context)
{
    ...
    // hasOwnProperty is called to prevent possible conflicts with prototype
	// extensions
...
Css.createStyleSheetMap = function(context)
{
    ...
    function addSheet(sheet)
    {
        ...
        for (var i = 0; i < sheet.cssRules.length; ++i)
        {
            ...
            {
                if (rule.type == CSSRule.STYLE_RULE)  // once we get here no
														// more imports
                    return;
            }
    ...
    Win.iterateWindows(context.window, function(subwin)
    {
        ...
        for (var i = 0; i < rootSheets.length; ++i)
        {
            ...
            {
                // TraceError.sysout("css.createStyleSheetMap; EXCEPTION " +
				// err, err);
            }
        }
    });
    ...
    for (var i=0, len=parts.length; i<len; ++i)
    {
        ...
        else
        {
            if (Css.isColorKeyword(part))
            {
                ...
                {
                    // Color keyword is a system color, which can't be resolved
					// by
                    // domUtils.colorNameToRGB(), so just return the keyword
					// itself
                    // (see issue 6753)
                    newValue += part;
                }
...
var cssDataExceptions =
{
    ...
    // special-cased by auto-completion, so "fontFamily" isn't included in the
	// list.
    // (For SVG presentational attribute completion it is wanted, but let's
	// ignore
...
var svgPresentationalProperties = new Set([
    ...
    // Unimplemented by Firefox, but it doesn't hurt to include them for future
	// compatibility.
    "alignment-baseline",
    "baseline-shift",
    "color-profile",
...
// Most common supported charsets according to
